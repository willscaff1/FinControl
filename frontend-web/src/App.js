...
const DashboardPage = () => {
  const navigate = useNavigate();
  const { openModal } = useContext(NewTransactionContext);
  ...
  const loadData = async (month = selectedMonth, year = selectedYear) => {
    try {
      ...
      // Carregar bancos
      const banksResponse = await axios.get('/api/banks');
          ...
          {banks.length > 0 ? (
            <div className="cards-grid">
              {banks.map((bank) => {
                ...
                const balance = (data.transactions || []).filter(t => 
                  t.bank === bank.name && 
      ...
      {showDeleteModal && transactionToDelete && (
        <div className="modal-overlay-focused">
          <div className="modal-content delete-modal">
            ...
            <div className="modal-body">
              <div className="transaction-info">
                <div className="transaction-details">
                  <span className="transaction-description">{transactionToDelete.description}</span>
                  <span className={`transaction-amount ${transactionToDelete.type}`}>
              ...
              {(transactionToDelete.recurringParentId || transactionToDelete.isRecurring) && (
                <>
                  <div className="warning-box recurring">
                    <div className="warning-icon">🔄</div>
                    <div className="warning-content">
                      <h4>Transação Recorrente</h4>
                      <p>Esta transação se repete mensalmente. O que deseja fazer?</p>
                    </div>
                  </div>
              ...
              {/* Transação Parcelada */}
              {(transactionToDelete.installmentParentId || transactionToDelete.isInstallment || transactionToDelete.installmentNumber > 0) && (
                <>
                  <div className="warning-box installment">
                    <div className="warning-icon">📊</div>
                    <div className="warning-content">
                      <h4>Transação Parcelada</h4>
                      {transactionToDelete.installmentNumber > 0 ? (
                        <p>Parcela {transactionToDelete.installmentNumber} de {transactionToDelete.totalInstallments}</p>
                      ) : (
                        <p>Esta transação está parcelada. O que deseja fazer?</p>
                      )}
                    </div>
                  </div>
            ...
            {/* Footer com botão cancelar para transações especiais */}
            {((transactionToDelete.recurringParentId || transactionToDelete.isRecurring) || 
              (transactionToDelete.installmentParentId || transactionToDelete.isInstallment || transactionToDelete.installmentNumber > 0)) && (
              <div className="modal-footer">
                <button 
...
const BanksPage = () => {
  ...
  // Calcular saldo acumulado histórico do banco (todas as transações)
  const calculateBankBalanceHistoric = (bankName) => {
    // Aqui poderia fazer uma chamada para buscar todas as transações históricas
    // Por enquanto, retornamos o mesmo valor do mês
    return calculateBankBalance(bankName);
  };
  ...
  const handleEditBank = async (bankData) => {
    try {
      await axios.put(`/api/banks/${selectedBank._id}`, bankData);
